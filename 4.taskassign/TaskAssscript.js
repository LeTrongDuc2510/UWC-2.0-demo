...
function slCnt(x) {
  ...
  if (document.getElementById("st-type").value == "jan") {
    stri = "Nhân viên quét dọn";
    bl1Choice = 1;
  } else {
    stri = "Nhân viên lái xe"
    bl1Choice = 2;
  }
  document.getElementById("Modal1-sl-count").innerHTML = "Đã chọn: " + x;
  if (x > 0)
  document.getElementById("bl1Txt").innerHTML = "Đã chọn: " + x + " / " + total/2 + " (" +  stri + ")";
  else 
...
function checkAll() {
  let btn = document.getElementById("modal1-row-0");
  checkFunc(btn.checked);
  if (btn.checked) slCnt(total); else slCnt(0);
}
...
function countRow() {
  ...
  if (cnt < total) btn.checked = false; else btn.checked = true;
  slCnt(cnt);
}
...
let maxPoint = 6;
let noPoint=0;
let finPoint = [1, 0, 0, 0, 0, 0];


function func(x) {
  document.getElementById("gen-shortest-path").style.display="";
  document.getElementById("show-OpMAP").style.display="none";
  document.getElementById("gen-shortest-path").style.opacity = 0.4;
    let countR = 0;
    const cbox = document.getElementsByClassName("cbox1");

    // if (btn1.checked || btn2.checked || btn3.checked) {
    //     s += "->";
    // } Chỉ có 1 thàng suy ra ko có dấu ->
    ...
    noPoint = countR+1; // for MainHouse
    

    if (countR >= 1) {
      document.getElementById('gen-shortest-path').style.pointerEvents = "auto";
      document.getElementById('gen-shortest-path').style.opacity = 1;
    } else {
    ...
    if (cbox[x].checked) {
      if (countR > 1) {
        ...
        s = ("<b><i>Đã chọn</i></b>: MCP" + (x+1));
      }
    } else if (!cbox[x].checked) 
      if (countR >= 1) {
          s = s.replace(", MCP" + (x+1), "");
          s = s.replace("MCP" + (x+1) + ", ", "");
          // alert(s);
      } else s = "Chọn <b>MCPs</b> để hình thành tuyến đường!";
    ...
    document.getElementById("Modal1-sl-countxx").innerHTML = "Đã chọn: " + countR;
}


function funcx(x) {

    const cbox = document.getElementsByClassName("cbox10");
    let countR = 0;
    // if (btn1.checked || btn2.checked || btn3.checked) {
    //     s += "->";
    // } Chỉ có 1 thàng suy ra ko có dấu ->
    ...
    if (cbox[x].checked) {
      if (countR > 1) {
        ...
        s = ("<b><i>Đã chọn</i></b>: MCP" + (x+1));
        if (x+1 == 1) {
          s+=" (497 Hòa Hảo, Phường 7, Quận 10, TPHCM).";
        }
        if (x+1 == 2) {
          s+=" (120 Hồng Bàng, Phường 12, Quận 5, TPHCM).";
        }

        if (x+1 == 3) {
          s+=" (520A Thành Thái, Phường 12, Quận 10, TPHCM).";
        }

        if (x+1 == 4) {
          s+=" (16A/A1 Lê Hồng Phong, Phường 12, Quận 10, TPHCM).";
        }

        if (x+1 == 5) {
          s+=" (279 Nguyễn Tri Phương, Phường 5, Quận 10, TPHCM).";
        }
      }
    } else if (!cbox[x].checked) 
      if (countR >= 1) {
          ...
          s = s.replace("MCP" + (x+1) + ", ", "");
          // alert(s);
      } else s = "Chọn <b>MCPs</b> để chỉ định cho Janitor!";

    document.getElementById("pathx").innerHTML = s;
}
...
function sortTable1(n) {
  ...
  while (switching) {
    ...
    for (i = 1; i < (rows.length - 1); i++) {
      ...
      /*Get the two elements you want to compare,
      one from current row and one from the next:*/
      x = rows[i].getElementsByClassName("modal2-tdTag")[n];
      y = rows[i + 1].getElementsByClassName("modal2-tdTag")[n];
      /*check if the two rows should switch place,
      based on the direction, asc or desc:*/
      if (dir == "asc") {
        if (x.innerHTML.toLowerCase() > y.innerHTML.toLowerCase()) {
...
function reEditTask() {
  ...
  const bgColor = document.getElementsByClassName('bg-color');
    ...
    bgColor[4].style.backgroundColor = "#f8f9fd";
  ...
  let checkboxes = document.querySelectorAll('input[name="modal1-tb-row"]');
  checkboxes[0].checked = true;
  document.getElementById("Modal1-sl-count").innerHTML = "Đã chọn: 1";
  document.getElementById("bl1Txt").innerHTML = "Đã chọn: 1" + " / " + total + " (nhân viên)";
  //  Modal2
  

  let checkboxes1 = document.querySelectorAll('input[name="tb1-row"]');
  checkboxes1[0].checked = true;


  noPoint=2;
  finPoint = [1, 1, 0, 0, 0, 0];

  document.getElementById("show-OpMAP").style.display="block";
  document.getElementById('gen-shortest-path').style.opacity = 0;
  document.getElementById("path").innerHTML = "Tuyến đường <b><i>tối ưu</i></b> (4.31 km): Trụ sở ⇨ MCP1 ⇨ Trụ sở";

  
  
  s = ("<b><i>Đã chọn</i></b>: MCP1");
  document.getElementById("dt-sl").value = "2023-11-22";
  document.getElementById("sh-sl").selectedIndex = 1;
  document.getElementsByClassName("bl-txt2")[1].innerHTML = "Ngày: 22/11/2023 » Ca 2 » Tuyến đường (4.31 km): Trụ sở ⇨ MCP1 ⇨ Trụ sở."
...
"Ngày: " + findate + "  »  " + document.getElementById("sh-slx").value + "  »  " + document.getElementById("ve-slx").value + "  »  " + strFin;
...
function alertxx() {
  ...
  if (bl2Choice == 1) {
    ...
    document.getElementsByClassName("tb-row-0xx")[4].innerHTML = "Quét dọn tại";
  }
  ...
  if (bl2Choice == 2) {
    ...
    document.getElementsByClassName("tb-row-0xx")[4].innerHTML = "Tuyến đường";
...
function delFUNC() {
  ...
  if (bl2Choice == 1) {
    for (let i=0; i<5; i++) {
      if (manCbox[i].checked == true) {
        cnt++;
        document.getElementsByClassName("manRowTab")[i].style.display = "none";
      };
    }

    document.getElementById("Modal1-page-countxxx").innerHTML = "1 - " + (JanTaskTotalCnt-cnt) + " trên " + (JanTaskTotalCnt-cnt);
  }
  ...
  if (bl2Choice == 2) {
    ...
    document.getElementById("Modal1-page-countxxx").innerHTML = "1 - " + (ColTaskTotalCnt-cnt) + " trên " + (ColTaskTotalCnt-cnt);
  }
